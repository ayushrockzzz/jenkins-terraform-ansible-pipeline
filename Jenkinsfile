pipeline {
  ...
  stages {
    ...
    stage('Wait EC2') {
      steps {
        sh '''
          aws ec2 wait instance-status-ok --region ap-south-1 --instance-ids `$(terraform output -json ec2_id_test) | awk -F'"' '{print $2}'`
        '''
      }
    }
    ...
    stage('Run Ansible') {
      steps {
        ansiblePlaybook(credentialsId: 'ec2.ssh.key', inventory: 'aws_hosts', playbook: 'ansible/docker.yml', hostKeyChecking: 'true')
      }
    }
